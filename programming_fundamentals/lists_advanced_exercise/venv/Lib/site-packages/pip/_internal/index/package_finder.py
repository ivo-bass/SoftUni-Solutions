...
class LinkEvaluator(object):
    """
    Responsible for evaluating links for a particular project_new.
    """
    ...
    # people_waiting when reading the code.
    def __init__(
            self,
    ...
    def evaluate_link(self, link):
        ...
        if link.egg_fragment:
            ...
            if ext == WHEEL_EXTENSION:
                ...
                if canonicalize_name(wheel.name) != self._canonical_name:
                    reason = 'wrong project_new name (not {})'.format(
                        self.project_name)
                    return (False, reason)
        ...
        if not version:
            reason = 'Missing project_new version for {}'.format(self.project_name)
            return (False, reason)
...
def filter_unallowed_hashes(
    ...
    if not hashes:
        logger.debug(
            'Given no hashes to check %s links for p%r: '
    ...
    logger.debug(
        'Checked %s links for p%r against %s hashes '
...
class PackageFinder(object):
    ...
    def process_project_url(self, project_url, link_evaluator):
        ...
        logger.debug(
            'Fetching project_new page and analyzing links: %s', project_url,
        )
    ...
    def find_best_candidate(
        ...
        # type: (...) -> BestCandidateResult
        """Find matches for the given project_new and specifier.
